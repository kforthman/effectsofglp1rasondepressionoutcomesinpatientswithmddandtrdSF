...
| [`ps_covariates.csv`](Data/ps_covariates.csv) | 36 | Propensity-score covariate list. Columns: `var`, `var_type` (`continuous` or `binary`). 6 continuous (time-from-diagnosis, age, year, and three utilization averages) and 30 binary covariates spanning demographics, comorbidities, psychiatric diagnoses, substance-use disorders, and baseline antidepressant use. Driven by `analysis_Propensity_Scoring.R`. |
...
- [`Data_Diary/prepData-schema.xlsx`](Data_Diary/prepData-schema.xlsx) — human-readable schema reference for the source tables.
