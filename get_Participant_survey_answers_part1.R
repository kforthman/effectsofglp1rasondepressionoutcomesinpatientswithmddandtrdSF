...
sql <- str_glue("SELECT DISTINCT question_concept_id
                                           FROM `{CURRENT_DATASET}.ds_survey` 
                                            WHERE survey IN ('Overall Health', 'Lifestyle', 
                                                'The Basics', 'Healthcare Access & Utilization')")
...
# %%
overwrite <- FALSE
if(!file.exists("combined_table_p1.csv") | overwrite){
    n <- nrow(questions)
    for(i in 1:n){#for(i in c(1,49:53)){
        flush.console()
        ...
        if(nrow(this_question_data_tokeep) > nrow(unique(this_question_data_tokeep[,c("person_id", "survey_datetime")]))){
            ...
            this_question_data_tokeep <- pivot_wider(this_question_data_tokeep,
                                                     ...
                                                     values_from = "is_true", 
                                                     values_fill = 0,
                                                     names_glue = paste0(as.character(question), ": {`", as.character(question), "`}"))
    ...
    allow.cartesian=TRUE)
    write.csv(combined_table, "combined_table_p1.csv", row.names = F)
}else{
    combined_table <- read.csv("combined_table_p1.csv")
}
...
this_mapping_qid <- data.frame(questions, name = questions_text)
this_rows <- data.frame(question_id = integer(),sourcevar=character(),answer=character())
...
# %%
my_cmd <- paste0("gsutil -q stat " , my_bucket, "/working_files/this_mapping_manualEdit.csv; echo $?")
file_exist = system(my_cmd, intern = T) == '0'
if(!file.exists("this_mapping_manualEdit.csv") | !file.exists("this_mapping_manualEdit_p1_2.csv")){
if(!file_exist){
    stop("Please upload the mapping file.")
}else{
    if(!file.exists("this_mapping_manualEdit.csv")){
    my_cmd <- paste0("gsutil -m cp ", my_bucket, "/working_files/this_mapping_manualEdit.csv", " ./")
    print(my_cmd)
    system(my_cmd)
    }
}}

var_mapping1 <- read.csv("this_mapping_manualEdit.csv")
...
data.frame(
    ...
    complete = unname(apply(combined_table_num, 2, function(x){round(sum(!is.na(x))/n,3)}))
           ...
           write.csv("combined_table_colnames_p1.csv")
...
# %%
write.csv(combined_table_num, "participant_survey_answers.csv")

# %%
write.csv(combined_table_num_collapse, "participant_survey_answers_collapsed.csv")

# %%
my_cmd <- paste0("gsutil rm ", my_bucket, "/participant_survey_answers.csv ")
#system(my_cmd)
my_cmd <- paste0("gsutil -m cp -r participant_survey_answers.csv ", my_bucket)
print(my_cmd)
#system(my_cmd)
