# -*- coding: utf-8 -*-
# get_Outcomes_MedChanges(): Computes antidepressant/antipsychotic medication
# change outcomes for all comparator groups and time periods.
#
# Arguments:
#   med_changes_file   — Path to the antidepressant/antipsychotic consecutive
#                        period .rds file. Must restore variable
#                        `consecutive_period_tab` with columns: PatientDurableKey,
...
#   column n_med_changes. Also saved to output_file as `outcomes`.

get_Outcomes_MedChanges <- function(med_changes_file,
                                    matched_data_files,
                                    period_info,
                                    target_drug,
                                    comparator_groups,
  ...
  load(med_changes_file)   # restores consecutive_period_tab
  ...
  for (group in comparator_groups) {
    ...
    # Pre-join med change records to matched cohort (done once per group)
    drugs_for_group <- consecutive_period_tab %>%
      filter(PatientDurableKey %in% matched.data$PatientDurableKey) %>%
      left_join(matched_slim, by = "PatientDurableKey") %>%
      mutate(
        time_from_index_days = as.numeric(difftime(first_record, index_date, units = "days")),
    ...
    for (j in seq_len(nrow(period_info))) {
      ...
      drugs_w <- drugs_for_group %>%
        filter(time_from_index_days >= bgn & time_from_index_days < end)

      med_changes <- drugs_w %>%
        group_by(PatientDurableKey, study_cohort) %>%
        summarize(n_med_changes = n(), .groups = "drop")

      k <- k + 1L
      results[[k]] <- these_ids %>%
        mutate(period = prd) %>%
        left_join(med_changes, by = c("PatientDurableKey", "study_cohort")) %>%
        mutate(n_med_changes = replace_na(n_med_changes, 0L))
    }
  }
