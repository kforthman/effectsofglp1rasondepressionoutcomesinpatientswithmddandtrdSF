...
all_ids <- read_table(config, col_schema, "mdd_data", conn = conProjects)[[1]]
set.seed(123)
all_ids_sample <- sample(all_ids, size = length(all_ids) * data_sample_size)
save(all_ids_sample, file = "OutputData/all_ids_sample.rds")
rm(all_ids)
gc()

med_index_table <- read_table(config, col_schema, "med_index_table", conn = conProjects) %>%
  filter(PatientDurableKey %in% all_ids_sample)
gc()
