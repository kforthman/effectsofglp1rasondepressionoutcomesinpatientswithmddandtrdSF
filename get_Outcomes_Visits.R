...
#                        variable `dte_cohort_visits` with columns: PatientDurableKey,
...
get_Outcomes_Visits <- function(visits_file,
  ...
  for (group in comparator_groups) {
    ...
    visits_for_group <- dte_cohort_visits %>%
      ...
      mutate(
        time_from_index_days = as.numeric(difftime(start_datetime, index_date, units = "days")),
    ...
    for (j in seq_len(nrow(period_info))) {
      ...
      visit_days <- visits_w %>%
        uncount(n_days) %>%
        group_by(PatientDurableKey, visit_occurrence_id, study_cohort) %>%
      ...
      results[[k]] <- these_ids %>%
        ...
        left_join(visit_days,   by = c("PatientDurableKey", "study_cohort")) %>%
        mutate(
