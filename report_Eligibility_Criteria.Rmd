...
diagnosis_timeline_data         <- params$diagnosis_timeline_data
nontreatment_group              <- params$nontreatment_group
diag_table                      <- params$diag_table
```
...
```{r build-dataset}
all.data <- params$dte_cohort_data2 %>%
  ...
  left_join(diag_table %>%
              ...
              pivot_wider(names_from = Diagnosis, values_from = FirstDiagnosisDate,
                          values_fill = NA, names_glue = "{Diagnosis}_Index"),
            by = "PatientDurableKey") %>%
  mutate(across(paste0(unique(diag_table$Diagnosis[diag_table$Diagnosis != "MDD"]), "_Index"),
                ~ if_else(is.na(.), FALSE, TRUE),
